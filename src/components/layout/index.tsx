...
const Layout: React.FunctionComponent = () => {
  ...
  const menuRoutes = useMemo(() => getVisibleRoutes(RoutesData), [pathname]);

  useEffect(() => {
    const token = getAccessToken();
    if (token) persistRolesFromToken(token);
  }, []);

  useEffect(() => {
    if (isExpertRole() && !isExpertAllowedPath(pathname)) {
      navigate(EXPERT_HOME_PATH, { replace: true });
    }
  }, [pathname, navigate]);
  ...
  const avatarMenuData = isExpertRole()
    ? [
        { icon: faUser, title: 'پروفایل کاربری', func: userProfile, id: 1 },
        { icon: faRightFromBracket, title: 'خروج', func: logout, id: 3 },
      ]
    : [
        { icon: faUser, title: 'پروفایل کاربری', func: userProfile, id: 1 },
        { icon: faGear, title: 'تنظیمات', func: settings, id: 2 },
        { icon: faRightFromBracket, title: 'خروج', func: logout, id: 3 },
      ];
  const Icon = imageUrl;
