...
const Customer: React.FunctionComponent = () => {
  ...
  // };
  const getAllUser = () => {
    instance.get(ApiHelper.get("getAllCustomers")).then((res:any) => {
  ...
  }, [showAddModal,showEditModal,showDeleteUser]);
  ...
  const columnDefs:ColDef[] = [
  ...
  
    {
      ...
      cellRenderer: (params:any) => {
        return (
          <span>{params.data.isActive ? "فعال" : "غیر فعال"}</span>
        )
      }
    },
  ...
  //  cellRenderer: (params:any) => {
  //   return (
  //     <Switch {...label} defaultChecked={params.data.isActive} onChange={() => setDisableUser(params)} />
  //   )
  ...
  <div className="flex justify-start items-start">
         ...
         <ShowForPermission role={["admin","superAdmin"]}>
                <button className='bg-yellow-500 text-xs py-2 cursor-pointer mr-2 rounded-md px-2  outline-none text-black' onClick={() => editUser(params)}>ویرایش مشتری</button>
                <button className='bg-red-500 mr-2 text-xs py-2 cursor-pointer rounded-md px-2  outline-none text-white' onClick={() => deleteUser(params)}>حذف مشتری</button>
    ...
    <Fragment>
       ...
       <div className="bg-white border border-[#2c3c511a] rounded-xl flex justify-between p-4 mb-3">
        <div className="flex items-center gap-3">
          <h3 className="text-base font-bold text-primary">  مشتریان </h3>
        </div>
      </div>
      <div className='mb-3'>
      <button className='bg-green-700 px-2  text-sm py-2 cursor-pointer mr-2 rounded-md   outline-none text-white' onClick={() => setShowAddModal(true)}>اضافه کردن مشتری</button>
      </div>
      ...
      {showAddModal && (
        <AddCustomer showAddUserModal={showAddModal} setShowAddUserModal={setShowAddModal} />
     )}
      {showEditModal && (
        <EditCustomer  customerId={userId} userName={userName} showEditUserModal={showEditModal} setShowEditUserModal={setShowEditModal}  />
     )}
      {showDeleteUser && (
       <DeleteCustomer customerId={userId} username={userName} showDeleteModal={showDeleteUser} setShowDeleteModal={setShowDeleteUser}/>
     )}
