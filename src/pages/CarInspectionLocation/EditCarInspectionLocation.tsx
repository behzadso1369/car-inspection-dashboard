...
const EditCarInspectionLocation: React.FunctionComponent<
EditPieceProps
> = ({ showEditModal, setShowEditModal,secretOfOurServiceQualityId,secretOfOurServiceQualityName }) => {
  ...
  instance.put(ApiHelper.get("EditCarInspectionLocation")+ "?id=" + secretOfOurServiceQualityId,formData).then((res:any) => {
    if(res.data) {
  ...
  const  getBlogTagById = () => {
    instance.get(ApiHelper.get("GetCarInspectionLocation"),{params:{id:secretOfOurServiceQualityId}}).then((res:any) => {
        reset({
          name:res.data.resultObject.name,
          locationTypeDescription:res.data.resultObject.locationTypeDescription,
...
export default EditCarInspectionLocation;
