...
const Order: React.FunctionComponent = () => {
  ...
  const columnDefs:ColDef[] = [
    ...
    },
    {
      ...
      cellRenderer: (params:any) => {
        return (
          <>
          {params.data.type === "sell" ? <button className='bg-red-500 text-white text-xs py-2 cursor-pointer mr-2 rounded-md px-2  outline-none '>فروش</button> :<button className='bg-green-600 text-xs py-2 cursor-pointer mr-2 rounded-md px-2  outline-none text-white'>خرید</button>}
          </>
        )
  ...
  return (
    <Fragment>
      ...
      <div style={gridStyle} className="ag-theme-quartz w-full">
        <AgGridReact
          ref={allgridRef}
