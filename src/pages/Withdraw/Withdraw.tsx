...
const Withdraw: React.FunctionComponent = () => {
  const containerStyle = useMemo(() => ({ width: '100%', height: '100%' }), []);
  const gridStyle = useMemo(() => ({ height: '100%', width: '100%' }), []);
  const [showDeleteWithdraw, setShowDeleteWithdraw] = useState<boolean>(false);
  ...
  }, [page,rowsPerPage,showDeleteWithdraw]);
  ...
  return (
    <Fragment>
       <div className="bg-white border border-[#2c3c511a] rounded-xl flex justify-between p-4 mb-3">
          ...
          <h3 className="text-base font-bold text-primary">لیست درخواست برداشت</h3>
          {/* <button className='bg-[#0047bc] px-2  text-sm py-2 cursor-pointer mr-2 rounded-md   outline-none text-white' onClick={() => setS(true)}>اضافه کردن حساب بانکی</button> */}
      ...
      {showDeleteWithdraw && (
       <DeleteWithdraw withdrawId={bankId} withdrawName={bankName} showDeleteModal={showDeleteWithdraw} setShowDeleteModal={setShowDeleteWithdraw}/>
     )}
