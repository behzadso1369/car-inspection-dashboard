...
const Services: React.FunctionComponent = () => {
  ...
  // };
  const getAllUser = () => {
    instance.get(ApiHelper.get("Service")).then((res:any) => {
      setRowData(res?.data)
    })
  }
  useEffect(() => {
  ...
  const columnDefs:ColDef[] = [
    ...
    },
    {
      ...
      cellRenderer: (params:any) => {
        return (
          <>
          {params.data.name ? <span className='text-sm'>{params.data.name}</span> : <button className='bg-slate-300 text-xs py-2 cursor-pointer mr-2 rounded-md px-2  outline-none text-black'>ثبت نشده است</button>}
          </>
        )
   ...
   
    {
      ...
      cellRenderer: (params:any) => {
        return (
          <>
          {params.data.small_name ? <span>{params.data.small_name}</span> :<button className='bg-slate-400 text-xs py-2 cursor-pointer mr-2 rounded-md px-2  outline-none text-black'>ثبت نشده است</button>}
          </>
        )
    ...
    {
      field: 'fix_fee',
      headerName: ' کارمزد ثابت'
    
    },
    {
        field: 'trade_fee',
        headerName: ' کارمزد معامله',
        cellRenderer: (params:any) => {
          return (
            <>
            {params.data.trade_fee ? <span className='text-base'>{params.data.trade_fee}</span> : <button className='bg-red-400 text-balck flex justify-center items-center py-2 px-3'>ثبت نشده است</button>}
            </>
          )
  ...
  <div className="flex justify-start items-start">
                ...
                <button className='bg-yellow-500 text-xs py-2 cursor-pointer mr-2 rounded-md px-2  outline-none text-black' ><NavLink to={"../../service/detail/" + params.data.id}>مشاهده</NavLink></button>
                <button className='bg-red-500 mr-2 text-xs py-2 cursor-pointer rounded-md px-2  outline-none text-white' >حذف </button>
  ...
  return (
    <Fragment>
       ...
       <div className="bg-white border border-[#2c3c511a] rounded-xl flex items-baseline justify-between p-4 mb-3">
          ...
          <h3 className="text-base font-bold text-primary">  سرویس ها </h3>
