...
interface DeleteFAQCategoryProps extends React.PropsWithChildren {
...
const DeleteFAQCategory: React.FunctionComponent<DeleteFAQCategoryProps> = ({
  showDeleteModal,
  setShowDeleteModal,
  categoryName,
  categoryId,
  ...
  const onSubmit = () => {
    instance.delete(ApiHelper.get('DeleteFAQCategories') + '?id=' + categoryId).then((res: any) => {
      if (res) {
  ...
  return (
    ...
    >
      <DialogTitle className="w-full flex items-center gap-3 border-b !pb-6">
        <span>حذف دسته بندی سوالات متداول</span>
      </DialogTitle>
