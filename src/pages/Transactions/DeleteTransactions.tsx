...
const DeleteTransactions: React.FunctionComponent<
EditPieceProps
> = ({ showDeleteModal, setShowDeleteModal,transactionName,transactionId }) => {



  const onSubmit = () => {
    instance.delete(ApiHelper.get("BankAccounts") + "/" + transactionId).then((res:any) => {
        if(res) {
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-3 pb-8 px-10 py-5">
        <div className='col-span-3'>
            <span>آیا از حذف تراکنش</span>
            <span className='text-blue-500'> {transactionName} </span>
            <span> </span>
