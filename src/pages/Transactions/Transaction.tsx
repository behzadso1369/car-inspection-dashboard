...
const Transaction: React.FunctionComponent = () => {
  ...
  const [showDeleteTranactions, setShowDeleteTranactions] = useState<boolean>(false);
  ...
  const deleteTreansactions = (params:any) => {
    setBankId(params.data.id);
    setBankName(params.data.user_full_name);
    setShowDeleteTranactions(true);
  ...
  const columnDefs:ColDef[] = [
   ...
   
    {
      ...
      cellRenderer: (params:any) => {
        return (
          <>
          {params.data.amount ? <span>{params.data.amount}</span> :<button className='bg-slate-400 text-xs py-2 cursor-pointer mr-2 rounded-md px-2  outline-none text-black'>ثبت نشده است</button>}
          </>
        )
  ...
  <div className="flex justify-start items-start">
                ...
                <button className='bg-yellow-500 text-xs py-2 cursor-pointer mr-2 rounded-md px-2  outline-none text-black' ><NavLink to={"../../transactions/detail/" + params.data.id}>جزییات تراکنش  </NavLink></button>
                {/* <button className='bg-red-500 mr-2 text-xs py-2 cursor-pointer rounded-md px-2  outline-none text-white' onClick={() => deleteTreansactions(params)}>حذف </button> */}
  ...
  }, [page,rowsPerPage,showDeleteTranactions]);
  ...
  return (
    <Fragment>
       <div className="bg-white border border-[#2c3c511a] rounded-xl flex justify-between p-4 mb-3">
          ...
          <h3 className="text-base font-bold text-primary">تراکنش ها</h3>
      ...
      {showDeleteTranactions && (
       <DeleteTransactions transactionId={bankId} transactionName={bankName} showDeleteModal={showDeleteTranactions} setShowDeleteModal={setShowDeleteTranactions}/>
     )}
