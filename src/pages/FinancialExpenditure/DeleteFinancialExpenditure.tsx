...
const DeleteFinancialExpenditure: React.FunctionComponent<
EditPieceProps
> = ({ showDeleteModal, setShowDeleteModal,secretOfOurServiceQualityName,secretOfOurServiceQualityId }) => {
  ...
  const onSubmit = () => {
    instance.delete(ApiHelper.get("DeleteFinancialExpenditure")+ "?id=" + secretOfOurServiceQualityId).then((res:any) => {
        if(res) {
  ...
  return (
    ...
    >
      <DialogTitle className="w-full flex items-center gap-3 border-b !pb-6">
        <span> حذف    خرجکرد مالی </span>
        
      </DialogTitle>
      <div className="grid grid-cols-3 gap-3 pb-8 px-10 py-5">
        <div className='col-span-3'>
            <span>آیا از حذف    حذف    خرجکرد مالی</span>
...
export default DeleteFinancialExpenditure;
