...
maxWidth:80,
  ...
  const cardFields = cardFieldsFromColDefs(columnDefs, {
    primaryField: 'carGroup',
    valueOverrides: {
      carGroup: (row: any) => row.carGroup?.name ?? '—',
      carInspectionType: (row: any) => row.carInspectionType?.name ?? '—',
      marketPrice: (row: any) => formatPrice(row.marketPrice),
      ourPrice: (row: any) => formatPrice(row.ourPrice),
    },
  });
